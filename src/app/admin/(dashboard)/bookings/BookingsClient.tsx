...
function AddLeadModal({ onClose, onSaved, showAds }: { onClose: () => void; onSaved: () => void; showAds: boolean }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <h2 className="font-headline text-xl text-[#cfe5fa]">Nuevo cliente</h2>
...
function BookingCards({
  bookings, updatingId, onUpdate, showAds,
}: {
...
function BookingsTable({ bookings, onRefresh, showAds }: { bookings: BookingRecord[], onRefresh: () => void, showAds: boolean }) {
  ...
  return (
    <section>
      ...
      <BookingCards bookings={bookings} updatingId={updatingId} onUpdate={handleUpdate} showAds={showAds} />
      ...
      <div className="hidden lg:block overflow-x-auto border border-[#42484c]/40 rounded-sm">
        <table className="w-full text-left text-sm font-body">
          ...
          <tbody>
            {bookings.length === 0 ? (
              <tr>
                <td colSpan={showAds ? 8 : 7} className="py-12 px-4 text-center text-[#8a9299]">
                  {EMPTY_MESSAGE}
                </td>
              </tr>
            ) : (
...
export default function BookingsClient({ role }: { role: AdminRole }) {
  // Las terapeutas ven solo la agenda: nada de GCLID, origen ni exportaciones.
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {showAddLead && (
        <AddLeadModal onClose={() => setShowAddLead(false)} onSaved={load} showAds={showAds} />
      )}
