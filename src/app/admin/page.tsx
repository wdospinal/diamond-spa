...
function LedgerTable({
  ...
  ledger: LedgerEntry[]
  ledgerModal: LedgerModalState | null
  setLedgerModal: (s: LedgerModalState | null) => void
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (ledgerModal?.mode === 'edit' && ledgerModal.entry.id === id) setLedgerModal(null)
      await load()
    } catch {
              ...
              ledger.map(e => (
                <tr key={e.id} className="border-b border-[#42484c]/25 text-[#cfe5fa] hover:bg-[#0a2438]/50">
                  ...
                  <td className="py-3 px-4 text-xs text-[#8a9299] leading-snug">
                    {e.kind === 'expense' ? getExpenseCategoryName(e.categoryId) ?? '—' : '—'}
                  </td>
                  <td className="py-3 px-4">
                    <DualCurrency usd={e.amount} copOverride={e.amountCop} tone={e.kind === 'income' ? 'income' : 'expense'} />
                  </td>
...
export default function AdminDashboardPage() {
  ...
  async function runExpenseSeed() {
    ...
    setSeeding(true)
    try {
      const res = await fetch('/api/admin/seed-expenses', { method: 'POST', credentials: 'same-origin' })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
      ...
      setNotice(data.skipped
        ? 'Los gastos iniciales ya estaban cargados (solo se puede hacer una vez).'
        : `Se agregaron ${data.inserted as number} movimientos de gastos iniciales.`)
      await load()
    } catch {
      setError('Error de red')
    } finally {
  ...
  if (bookings === undefined && !error) {
    return <div className="min-h-screen flex items-center justify-center font-body text-[#8a9299]">Cargando…</div>
  }
  ...
  return (
    <div className="min-h-screen px-6 pt-14 pb-10 md:px-12 md:pt-16 md:pb-12 max-w-6xl mx-auto">
      <AdminHeader
        onAddIncome={() => setLedgerModal({ mode: 'create', kind: 'income' })}
        onAddExpense={() => setLedgerModal({ mode: 'create', kind: 'expense' })}
        seeding={seeding}
        seedExpensesDone={seedExpensesDone}
        onSeedExpenses={() => void runExpenseSeed()}
    ...
    <div className="bg-[#0a2438] border border-[#42484c]/30 p-6">
      ...
      <dl className="space-y-3 text-sm font-body">
        <div className="flex justify-between gap-4 items-start">
          ...
          <dd><DualCurrency usd={p.sessionsIncome} tone="muted" /></dd>
        </div>
        <div className="flex justify-between gap-4 items-start">
          <dt className="text-[#8a9299] shrink-0 pt-0.5">Otros ingresos</dt>
          <dd><DualCurrency usd={p.otherIncome} tone="income" /></dd>
        </div>
        <div className="flex justify-between gap-4 items-start">
          <dt className="text-[#8a9299] shrink-0 pt-0.5">Gastos</dt>
          <dd><DualCurrency usd={p.expenses} tone="expense" /></dd>
        </div>
        <div className="flex justify-between gap-4 items-start pt-2 border-t border-[#42484c]/40">
          ...
          <dd><DualCurrency usd={p.totalIncome} tone="income" /></dd>
        </div>
        <div className="flex justify-between gap-4 items-start">
          <dt className="font-label text-[10px] uppercase tracking-widest text-on-surface shrink-0 pt-1">Balance neto</dt>
          <dd><DualCurrency usd={p.net} tone={p.net >= 0 ? 'netPositive' : 'netNegative'} prominent /></dd>
        </div>
      </dl>
    </div>
  )
...
function ExpenseFields({
  ...
  amountCopStr: string
  onAmountCopChange: (s: string) => void
}) {
  return (
    <>
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          inputMode="numeric"
          value={amountCopStr}
          onChange={e => onAmountCopChange(e.target.value)}
...
function LedgerModal({
  ...
  const title =
    state.mode === 'edit' ? 'Editar movimiento'
    : state.kind === 'income' ? 'Registrar ingreso'
    : 'Registrar gasto'

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const n = Number(String(amount).replace(',', '.'))
    if (kind === 'income' ? (!Number.isFinite(n) || n <= 0) : (!Number.isFinite(n) || n < 0)) return

    const parsedCop = parseCopDigits(amountCopStr)
    let amountCop: number | null | undefined
    if (kind === 'expense') {
      if (parsedCop !== undefined) amountCop = parsedCop
      else if (state.mode === 'edit' && state.entry.amountCop != null && amountCopStr.trim() === '')
        amountCop = null
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
      ...
      >
        <h2 id="ledger-modal-title" className="font-headline text-xl text-[#cfe5fa] mb-6">{title}</h2>
        <form onSubmit={e => void handleSubmit(e)} className="flex flex-col gap-5">
          <div className="flex flex-col gap-1">
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 font-body text-sm text-[#cfe5fa] cursor-pointer">
                <input type="radio" name="ledger-kind" checked={kind === 'income'} onChange={() => setKind('income')} className="accent-[#a5cce6]" />
                Ingreso
              </label>
              <label className="flex items-center gap-2 font-body text-sm text-[#cfe5fa] cursor-pointer">
                <input type="radio" name="ledger-kind" checked={kind === 'expense'} onChange={() => setKind('expense')} className="accent-[#c97b63]" />
                Gasto
              </label>
            </div>
          </div>
          ...
          {kind === 'expense' ? (
            <ExpenseFields
              ...
              amountCopStr={amountCopStr}
              onAmountCopChange={setAmountCopStr}
            />
          ) : null}
          <div className="flex flex-col gap-1">
            <label htmlFor="dk" className="font-label text-xs text-[#8a9299] uppercase tracking-widest">Fecha</label>
            <input
              id="dk"
              type="date"
              required
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="nt" className="font-label text-xs text-[#8a9299] uppercase tracking-widest">Nota (opcional)</label>
            <input
              id="nt"
              type="text"
              value={note}
