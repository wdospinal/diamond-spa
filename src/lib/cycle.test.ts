...
test('cierre 25: agosto va del 26 de julio al 25 de agosto', () => {
  assert.deepEqual(cycleRange('2026-08', 26), { from: '2026-07-26', to: '2026-08-25' })
})

test('cierre 25: el período cruza el cambio de año', () => {
  assert.deepEqual(cycleRange('2026-01', 26), { from: '2025-12-26', to: '2026-01-25' })
})
...
test('a qué período pertenece cada día, con cierre 25', () => {
  // El día del corte ya cuenta para el período siguiente.
  assert.equal(cycleLabelFor('2026-07-25', 26), '2026-07')
  assert.equal(cycleLabelFor('2026-07-26', 26), '2026-08')
  assert.equal(cycleLabelFor('2026-08-25', 26), '2026-08')
  assert.equal(cycleLabelFor('2026-08-26', 26), '2026-09')
  // Fin de año.
  assert.equal(cycleLabelFor('2026-12-26', 26), '2027-01')
})

test('el rango y la pertenencia son coherentes entre sí', () => {
  // Todo día dentro del rango de un período debe mapear a ese período.
  for (const startDay of [1, 5, 15, 26, 28]) {
...
test('el día de corte se recorta a un rango seguro', () => {
  ...
  assert.equal(normalizeStartDay('26'), 26)
  assert.equal(normalizeStartDay('no es un número'), 26)
  assert.equal(normalizeStartDay(undefined), 26)
})

test('la etiqueta del rango se lee sin ambigüedad', () => {
  assert.equal(cycleRangeLabel('2026-08', 26), '26 jul – 25 ago')
  assert.equal(cycleRangeLabel('2026-08', 1), '1 ago – 31 ago')
})

test('compara los mismos días transcurridos entre ciclos', () => {
  assert.deepEqual(cycleComparison('2026-09-07', 26), {
    currentLabel: '2026-09',
    previousLabel: '2026-08',
    current: { from: '2026-08-26', to: '2026-09-25' },
    previous: { from: '2026-07-26', to: '2026-08-25' },
    elapsedDays: 13,
